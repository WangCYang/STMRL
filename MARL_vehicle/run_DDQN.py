...
if __name__ == "__main__":
    ...
    for i in Edge_IDs:
        ...
        rl_agent = RL_agent_DDQN(num_car_max*3 + num_car_max* 3 + 2 + 1 + len(edge_i.neighboring_edges)*3,  # state： 自身状态（任务信息，车辆信息，边缘服务器信息，流量预测） + 邻居节点的状态(任务总量, 算力，预测)
                                 ...
                                 learning_rate, batch_size, "{}/v1_hidden_{}_bs_{}_epoch_{}".format(Simulation_scenario,num_hidden_nodes1,batch_size,epoches),i)          # new an agent
    ...
    # avr_per_task = np.zeros(epoches*episode)  # 记录每次episode的结果
    avr_per_task = []
    epi_count = 0
    ...
    saved_avg_data = "saved_data/{}/MARL_training_{}_v1_hidden_{}_bs_{}_epoch_{}.pkl".format(Simulation_scenario,Time_range,num_hidden_nodes1, batch_size, epoches)
    check_and_create_path(saved_avg_data)
    fw = open(saved_avg_data, "wb")
    pickle.dump(np.array(avr_per_task), fw)
    fig(avr_per_task,'Train Average Latency Per Task') # 画出曲线