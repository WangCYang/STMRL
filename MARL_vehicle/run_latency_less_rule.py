...
if __name__ == "__main__":
    ...
    for epd in range(start_epi, start_epi + episode):
        ...
        while True:
            ...
            for i in Edge_IDs:
                edge_i = e.edges[i]
                edge_i_decisions = np.zeros(edge_i.car_max)
                edge_wait_latency = 0
                if edge_i.buff_CPU > 0:  # 边缘计算是否等待
                    edge_wait_latency = edge_i.buff_CPU / edge_i.cpu
                for k in range(len(edge_i.connected_vehicles)): # 判断每个车辆的自身等待时延是否大于edge等待时延
                    if k >= edge_i.car_max:   # 连接车辆超过最大车辆数，break
                        break
                    vehicle_k = e.vehicle_pool[edge_i.connected_vehicles[k]]
                    car_wait_latency = 0
                    if vehicle_k.buff_CPU > 0:  # 本地计算是否等待
                        car_wait_latency = vehicle_k.buff_CPU / vehicle_k.cpu
                    if edge_wait_latency <= car_wait_latency:  # 边缘等待时延更小时，卸载到边缘
                        edge_i_decisions[k] = 1
                sum = 0
                for k in range(edge_i.car_max):
                    sum += edge_i_decisions[k]*int(math.pow(2,k))
                actions[i] = int(sum)
    ...
    saved_avr_laten_file = "saved_data/{}/Wait_less_test_range_{}_{}.pkl".format(Simulation_scenario, start_epi,
                                                                              start_epi + episode)
    ...
    saved_fig_file = "figv0/{}/{}_Wait_less_range_{}_{}.png".format(Simulation_scenario, flag, start_epi,
                                                                 start_epi + episode)
