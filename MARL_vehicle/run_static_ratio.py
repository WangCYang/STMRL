...
if __name__ == "__main__":
    ...
    for epd in range(start_epi, start_epi + episode):
        ...
        while True:
            ...
            for i in Edge_IDs:
                ...
                if len(e.edges[i].connected_vehicles) < e.edges[i].car_max:
                    offload_num_i = int(len(e.edges[i].connected_vehicles)* fixed_ratio)
                else:
                    offload_num_i = int(e.edges[i].car_max * fixed_ratio)

                sum = 0
                for k in range(offload_num_i):
    ...
    saved_avr_laten_file = "saved_data/{}/Static_{}_test_range_{}_{}.pkl".format(Simulation_scenario,fixed_ratio, start_epi,
                                                                              start_epi + episode)
    ...
    saved_fig_file = "figv0/{}/{}_Static_{}_range_{}_{}.png".format(Simulation_scenario, flag,fixed_ratio, start_epi,
                                                                 start_epi + episode)
    check_and_create_path(saved_fig_file)
    fig2plot(episode, avr_per_task, avr_wait_per_task, flag, saved_fig_file)  # 画出曲线画出曲线